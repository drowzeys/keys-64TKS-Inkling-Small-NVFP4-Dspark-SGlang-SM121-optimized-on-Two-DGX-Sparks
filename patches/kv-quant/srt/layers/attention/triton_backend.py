...
class TritonAttnBackend(AttentionBackend):
    ...
    def __init__(
        ...
        # sgl-project/sglang#30555: the DSpark proposer emits gamma rows per seq
        # on the DRAFT worker; the inherited server value is the target's gamma+1.
        if (
            self.num_draft_tokens
            and getattr(model_runner, 'is_draft_worker', False)
            and getattr(model_runner, 'spec_algorithm', None) is not None
        ...
        # ---- block-scaled quantized KV cache detection (triton lane) ------
        # Design: q/k/v stay bf16 through the kernels. K/V are quantized at
        # KV-store time BY THE POOL (see srt/mem_cache/kv_quant_pools.py, so
        # that every writer -- this backend, the DSpark hidden-state injector,
        # the verify prefix-valid commit -- gets scales), and the decode/extend
        # kernels dequantize pool reads per tile via the scale buffers from
        # get_kv_scale_buffer. Two recipes share one kernel module:
        #   mxfp8            fp8-e4m3 payload, one UE8M0 scale / 32 elements
        #   fp4_mx_block16   packed e2m1 payload, one UE8M0 scale / 16 elements
        #                    (what --kv-cache-dtype nvfp4 resolves to here)
        # All guards below fail at BOOT with actionable messages instead of
        # producing silent garbage downstream. When the pool is neither,
        # self.use_kv_mxfp8 / self.use_kv_nvfp4 are False and every new code
        # path is inert.
        if isinstance(self.token_to_kv_pool, SWAKVPool):
            _kv_sub_pools = [
                self.token_to_kv_pool.full_kv_pool,
                self.token_to_kv_pool.swa_kv_pool,
            ]
        else:
            _kv_sub_pools = [self.token_to_kv_pool]
        self.use_kv_mxfp8 = any(
            isinstance(p, MHATokenToKVPoolMXFP8) for p in _kv_sub_pools
        )
        self.use_kv_nvfp4 = any(
            isinstance(p, MHATokenToKVPoolFP4) for p in _kv_sub_pools
        )
        if self.use_kv_mxfp8 and self.use_kv_nvfp4:
            raise NotImplementedError(
                "mixed mxfp8 / fp4 KV sub-pools are not supported; got "
                f"{[type(p).__name__ for p in _kv_sub_pools]}."
            )
        self.use_kv_quant = self.use_kv_mxfp8 or self.use_kv_nvfp4
        if self.use_kv_quant:
            _want = MHATokenToKVPoolFP4 if self.use_kv_nvfp4 else MHATokenToKVPoolMXFP8
            if not all(isinstance(p, _want) for p in _kv_sub_pools):
                raise NotImplementedError(
                    "block-scaled quantized KV cache with the triton backend "
                    f"requires ALL attention sub-pools to be {_want.__name__}; "
                    f"got {[type(p).__name__ for p in _kv_sub_pools]}."
                )
            if self.use_kv_nvfp4:
                for p in _kv_sub_pools:
                    if not hasattr(p, "get_kv_scale_buffer"):
                        raise NotImplementedError(
                            "fp4 KV cache with the triton backend requires the "
                            "raw-reading pool subclass "
                            "(srt/mem_cache/kv_quant_pools.MHATokenToKVPoolFP4"
                            "Triton); the stock MHATokenToKVPoolFP4 dequantizes "
                            "the WHOLE pool on every read."
                        )
            for p in _kv_sub_pools:
                if getattr(p, "mxfp8_sf_interleaved", False):
                    raise NotImplementedError(
                        "mxfp8 KV cache with the triton backend supports only "
                        "the flat per-slot scale layout (page_size=1); the "
                        "fa4-interleaved page_size=128 BlockScaledBasicChunk "
                        "layout is not readable by the triton kernels. "
                        "Use --page-size 1."
                    )
            if self.page_size != 1:
                raise NotImplementedError(
                    "quantized KV cache with the triton backend supports only "
                    f"page_size=1, got page_size={self.page_size}."
                )
            if self.use_mla:
                raise NotImplementedError(
                    "quantized KV cache with the triton backend does not support "
                    "MLA models."
                )
            if self.dcp_size > 1:
                raise NotImplementedError(
            ...
            if self.enable_deterministic:
                raise NotImplementedError(
                    "quantized KV cache with the triton backend does not support "
                    "--enable-deterministic-inference (the unified extend "
                    "kernel has no dequant path)."
                )
            # The quantized read path lives in a separate module with
            # dedicated kernel clones; the upstream decode/extend kernel
            # modules stay byte-identical and this import only happens when
            # the pool IS quantized (under kv=auto it is never imported).
            from sglang.kernels.ops.attention.kv_quant_attention import (
                decode_attention_fwd_kv_quant,
                extend_attention_fwd_kv_quant,
            )

            self.decode_attention_fwd_kv_quant = torch.compiler.disable(
                decode_attention_fwd_kv_quant
            )
            self.extend_attention_fwd_kv_quant = torch.compiler.disable(
                extend_attention_fwd_kv_quant
            )
        # Config provenance for boot-test triage (grep-able): with
        # use_kv_mxfp8=False this backend calls the UNMODIFIED kernel modules
        # with the byte-identical upstream argument lists (zero new kwargs,
        # zero new specialization), and the mxfp8 module is never imported.
        logger.info(
            "TritonAttnBackend init: use_kv_mxfp8=%s use_kv_nvfp4=%s "
            "page_size=%s is_draft_worker=%s pools=%s",
            self.use_kv_mxfp8,
            self.use_kv_nvfp4,
            self.page_size,
            getattr(model_runner, "is_draft_worker", False),
            [type(p).__name__ for p in _kv_sub_pools],
        )
    ...
    @staticmethod
    def _reject_descale_kwargs(q_descale, k_descale, v_descale):
        """The triton mxfp8 lane expects bf16 q/k/v: KV quantization happens at
        store time and Q is never quantized. A non-None descale kwarg means the
        mxfp8 pre-quant block in inkling_common/attn.py lost its fa4-only gate
        — attending with pre-quantized fp8 tensors here would double-scale."""
        if q_descale is not None or k_descale is not None or v_descale is not None:
            raise NotImplementedError(
                "TritonAttnBackend received mxfp8 descale kwargs "
                "(q_descale/k_descale/v_descale). The triton backend expects "
                "bf16 q/k/v — KV is quantized at store time by the backend "
                "itself. Keep the mxfp8 pre-quantization in "
                "sglang/srt/models/inkling_common/attn.py gated to "
    ...
    def forward_extend(
        ...
        self._reject_descale_kwargs(q_descale, k_descale, v_descale)
        ...
        if k is None and v is None:
            if self.use_kv_quant:
                raise NotImplementedError(
                    "k/v re-read from a block-scaled quantized KV pool returns "
                    "the raw fp8/packed-e2m1 payload without its block scales; "
                    "pass k/v explicitly."
                )
            ...
            if save_kv_cache:
                ...
                if self.use_kv_quant:
                    # Quantize-at-store happens INSIDE the pool
                    # (srt/mem_cache/kv_quant_pools.py) so that this backend,
                    # the DSpark hidden-state injector and the verify
                    # prefix-valid commit all write payload AND block scales.
                    # k/v stay bf16 here for the kernel's current-chunk stage.
                    if layer.k_scale is not None:
                        raise NotImplementedError(
                            "per-tensor layer.k_scale/v_scale cannot be "
                            "combined with a block-scaled quantized KV cache."
                        )
        ...
        if (
            self.use_verify_splitkv
            # verify_splitkv reads the raw KV pool without block-scale dequant;
            # under mxfp8 fall through to the (dequantizing) extend kernel.
            and not self.use_kv_quant
        ...
        if self.use_kv_quant:
            # Dedicated wrapper + kernel clone. Scale buffers are routed
            # full/SWA sub-pool by layer_id exactly like the payload buffers,
            # so (window_)kv_indices index them identically.
            k_sf_buffer, v_sf_buffer = self.token_to_kv_pool.get_kv_scale_buffer(
                layer.layer_id
            )
            self.extend_attention_fwd_kv_quant(
                q.view(-1, layer.tp_q_head_num, layer.qk_head_dim),
                k.contiguous(),
                v.contiguous(),
                o.view(-1, layer.tp_q_head_num, layer.v_head_dim),
    ...
    def forward_decode(
        ...
        if save_kv_cache:
            ...
            elif self.use_kv_quant:
                # The pool quantizes at store (kv_quant_pools.py): payload +
                # block scales in one write, for every writer. q/k/v stay bf16.
                if layer.k_scale is not None:
                    raise NotImplementedError(
                        "per-tensor layer.k_scale/v_scale cannot be combined "
                        "with a block-scaled quantized KV cache."
                    )
                self._set_kv_buffer(
                    forward_batch,
                    layer,
        ...
        if self.use_kv_quant:
            # Dedicated wrapper + kernel clones; scale buffers share the
            # payload buffers' sub-pool routing and slot indexing.
            k_sf_buffer, v_sf_buffer = self.token_to_kv_pool.get_kv_scale_buffer(
                layer.layer_id
            )
            self.decode_attention_fwd_kv_quant(
                q.view(-1, layer.tp_q_head_num, layer.qk_head_dim),
                self.token_to_kv_pool.get_key_buffer(layer.layer_id),
                self.token_to_kv_pool.get_value_buffer(layer.layer_id),
                o.view(-1, layer.tp_q_head_num, layer.v_head_dim),
