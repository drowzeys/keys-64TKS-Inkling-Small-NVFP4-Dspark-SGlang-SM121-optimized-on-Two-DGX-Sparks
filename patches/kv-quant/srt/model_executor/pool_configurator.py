...
class HybridSWAPoolConfigurator(MemoryPoolConfigurator):
    ...
    def __init__(self, kvc: KVCacheConfigurator):
        ...
        if is_float4_e2m1fn_x2(kv_cache_dtype):
            # torch._utils._element_size(float4_e2m1fn_x2) is 1, but the pool
            # stores two e2m1 values per byte plus one uint8 block scale per 16
            # elements -> 0.5625 B/elem. Without this branch the hybrid-SWA
            # budget over-counts fp4 by 1.78x and the token capacity lands at
            # the mxfp8 multiplier instead of 3.556x bf16.
            scale_block_size = 16
            self._full_per_token = (
                self._full_per_token // 2 + self._full_per_token // scale_block_size
            )
            self._swa_per_token = (
                self._swa_per_token // 2 + self._swa_per_token // scale_block_size
            )
        elif get_model().kv_cache_dtype == "mxfp8":
            scale_block_size = 32
            self._full_per_token += (
