...
class KVCacheConfigurator:
    ...
    def _triton_quant_pool_class(self):
        ...
        ``fp4_mx_block16`` had none reachable for the Inkling hybrid-SWA layout
        (``MHATokenToKVPoolFP4`` is only built by the dead
        ...
        if dtype_str in ("nvfp4", "fp4_mx_block16") and is_float4_e2m1fn_x2(
            ...
            if self.post_capture_kv_active:
                raise NotImplementedError(
                    "post-capture KV backing is not supported with fp4 KV "
                    "cache (the scale buffers are not part of the VA-backed "
                    "payload)."
                )
    ...
    def _build_hybrid_swa_kv_pool(
        ...
        *,
        ...
        # Upstream routes ONLY mxfp8 to a block-scaled class here; there was no
