...
class InklingAttention(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        extra_attn_kwargs = {}
        # The mxfp8 pre-quantization + descale-kwarg contract below is FA4-ONLY:
        # the triton lane keeps q/k/v bf16 end-to-end. On triton the backend
        # quantizes K/V at KV-store time (MHATokenToKVPoolMXFP8 flat page_size=1
        # layout) and the triton kernels dequantize pool reads with the UE8M0
        # scale buffers, so no q/k/v_descale kwargs must be sent there — the
        # triton backend raises NotImplementedError if it receives any.
        if server_args.kv_cache_dtype == "mxfp8" and fa4:
